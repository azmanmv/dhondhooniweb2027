    ...
    <div class="min-h-screen bg-gray-100 flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
      <div class="max-w-md w-full space-y-8 bg-white p-8 rounded-xl shadow-md">
        <div>
          ...
          <p class="mt-2 text-center text-sm text-gray-600">Enter password 'admin' to access dashboard</p>
        </div>
        <form class="mt-8 space-y-6" (ngSubmit)="onLogin()">
          <div class="rounded-md shadow-sm -space-y-px">
            <div>
              <label for="password" class="sr-only">Password</label>
              <input id="password" name="password" type="password" [(ngModel)]="password" required class="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-md focus:outline-none focus:ring-blue-500 focus:border-blue-500 focus:z-10 sm:text-sm" placeholder="Password">
            </div>
          </div>
        ...
        <div class="text-center">
            <a routerLink="/" class="text-sm text-blue-600 hover:underline">Back to Website</a>
        </div>
      </div>
    </div>
  `
